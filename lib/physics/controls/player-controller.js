...
class PlayerControls extends Controls {
	...
	update(dt, t) {
		...
		if (this.keyResolutions.size > 0) {
			this.keyResolutions.forEach(element => {
				if (this.frameActions.has(element))
					this.resolveAction(element)
			});
		}
	...
	waitForKeyResolution(key) {
		if (key in KeyActions)
			this.keyResolutions.add(key)
	}

	resolveAction(key) {
		SceneManager.emit(`resolve-action-${key}`)
	}
