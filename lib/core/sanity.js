...
function SanityMeter() {
    ...
    this.createPoints = function (amount = 4, startOffset = 0.1) {
        ...
        if (pathConfig.points.length > 0 && true) {
            const randomIndex = Math.floor(Math.random() * 6)
            const newPoints =
                randomIndex == 0 ? [0.25, 0.5, 0.75]
                    : randomIndex == 1 ? [0.375, 0.4, 0.4, 0.75]
                        : randomIndex == 2 ? [0.333, 0.5, 0.666,]
                            : randomIndex == 3 ? [0.125, 0.25, 0.5, 0.625, 0.75]
                                : randomIndex == 4 ? [0.5, 0.6, 0.7, 0.8]
                                    : [0.5];
            newPoints.forEach(offset => {
    ...
    this.doBeep = function () {
        ...
        if (pointHit) {
            ...
            this.errors += 1;
            this.sanity -= Math.log(this.errors) * 1.5;
    ...
    this.endLevel0 = function () {
        ...
        this.gameLevel = 1
        SceneManager.emit('game-level-change', this.gameLevel)
        this.setBPM(130 / 8)
        this.timer = 0;
        this.cycles = 0
        this.errors = 0
