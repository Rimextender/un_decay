...
const itskaModel = 'assets/models/itsuka.glb'
...
function HallwayScene() {
    ...
    const itskaActor = new Actor({
        model: itskaModel,
        animations: [NoahTwoAnimation.idle, NoahTwoAnimation.walk],
        position: new THREE.Vector3(0, -10, 0)
    })
    itskaActor.mesh.position.x += 1
    itskaActor.mesh.position.z -= 1
    itskaActor.mesh.visible = false
    noahActor.mesh.add(itskaActor.mesh)
    ...
    rileyActor.mesh.position.x -= 1
    rileyActor.mesh.position.z -= 8
    rileyActor.mesh.position.y = 1
    rileyActor.mesh.visible = false
    noahActor.mesh.add(rileyActor.mesh)
    ...
    this.update = function (dt, t) {
        console.log(rileyActor.mesh.rotation);
    ...
    this.onLoad = async function () {
        ...
        SceneManager.listen('game-level-change', e => {
            const level = e.detail
            console.log('asdsad');

            if (level == 1) {
                rileyActor.mesh.visible = true
            }
            else if (level == 2) {
                itskaActor.mesh.visible = true
            }
        })
